...
class CRM_CivirulesActions_Contact_SyncInvestor extends CRM_Civirules_Action
{
    ...
    public function processAction(CRM_Civirules_TriggerData_TriggerData $triggerData)
    {
        ...
        if ($triggerData->getEntity() == 'Relationship') {
            $relationship = $triggerData->getEntityData('Relationship');
            $this->updateOrganizationFromRelationship($relationship);
        }
    }
...
// Get the results from the Api4\Generic\Result Class
    ...
    /*
     * Updates the custom fields on the organization.
     */
    ...
    /**
     * Updates the Organization from relationship data.
     */
    private function updateOrganizationFromRelationship(array $relationship): void
    {
        if ($relationship && array_key_exists('contact_id_b', $relationship)) {
            $relationshipTypes = \Civi\Api4\RelationshipType::get(TRUE)
                ->addSelect('*')
                ->addWhere('id', '=', $relationship['relationship_type_id'])
                ->execute()
                ->first();
            if ($relationshipTypes['name_a_b'] == 'Investor at') {
                $contId = $relationship['contact_id_b'];
                $this->syncOrganizationCustomFields($contId);
            }
        }
    }
